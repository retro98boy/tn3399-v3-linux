...
- 支持1920x1080 60Hz 双通道8bit的屏幕（未验证）

- 使用哪种LVDS屏幕通过不同的设备树选择
...
下载主线内核源码，添加dts和TC358775驱动（参考linux6.5.2-add-tn3399-v3-support.patch），配置内核选项时勾选上：
...
- 得到设备树`rk3399-tn3399-v3.dtb` `rk3399-tn3399-v3-with-1024x600-single-8bit-lvds-panel.dtb` `rk3399-tn3399-v3-with-1920x1080-dual-8bit-lvds-panel.dtb`，当然也可以直接使用仓库中预编译的设备树
...
- HDMI屏幕加1920x1080 60Hz 双通道 8bit LVDS屏幕
...
先将Manjaro-ARM系统的设备树替换成仓库中的rk3399-tn3399-v3-with-1024x600-single-8bit-lvds-panel.dtb（取决于使用哪种LVDS屏幕）
...
将仓库中的panel-toshiba-tc358775-2.0.1目录上传到Manjaro-ARM中，执行：
...
将panel-toshiba-tc358775-2.0.1放到/usr/src下，执行：

```
# 安装工具
sudo pacman -S dkms linux-headers gcc make
# 将模块纳入DKMS管理，每当内核有变动时，会自动编译安装
sudo dkms add -m panel-toshiba-tc358775 -v 2.0.1
# 不想等内核变动，立刻安装
sudo dkms install -m panel-toshiba-tc358775 -v 2.0.1
```
